...
def departamento_vacio():
    return Departamento("Departamento Vacío")
...
def test_dar_departamento(censo):
    ...
    assert censo.darDepartamento(31).darNombre() == "Vichada", "El nombre del último departamento es incorrecto"

def test_dar_ingreso_promedio(censo, departamento_lleno):
    censo.departamentos[0] = departamento_lleno
    assert censo.darIngresoPromedio() == 22500, "El ingreso promedio del país es incorrecto"

def test_dar_poblacion(censo, departamento_lleno):
    censo.departamentos[0] = departamento_lleno
    assert censo.darPoblacion() == 3000, "La población total del país es incorrecta"

def test_dar_temperatura_media(censo, departamento_lleno):
    censo.departamentos[0] = departamento_lleno
    assert censo.darTemperaturaMedia() == 27, "La temperatura media del país es incorrecta"

def test_dar_total_hombres(censo, departamento_lleno):
    censo.departamentos[0] = departamento_lleno
    assert censo.darTotalHombres() == 1500, "El total de hombres del país es incorrecto"

def test_dar_total_mujeres(censo, departamento_lleno):
    censo.departamentos[0] = departamento_lleno
    assert censo.darTotalMujeres() == 1500, "El total de mujeres del país es incorrecto"

def test_dar_edad_promedio(censo, departamento_lleno):
    censo.departamentos[0] = departamento_lleno
    assert censo.darEdadPromedio() == 32, "La edad promedio del país es incorrecta"
...
def test_existen_dos_dptos_igual_poblacion(censo, departamento_lleno):
    ...
    assert censo.existenDosDptosIgualPoblacion() == True, "Debe haber dos departamentos con igual población"

def test_metodo1(censo):
    assert censo.metodo1() == "Respuesta 1", "La respuesta del método 1 es incorrecta"

def test_metodo2(censo):
    assert censo.metodo2() == "Respuesta 2", "La respuesta del método 2 es incorrecta"
